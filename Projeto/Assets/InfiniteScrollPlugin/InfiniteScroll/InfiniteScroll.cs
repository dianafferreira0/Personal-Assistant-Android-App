...
namespace Mopsicus.InfiniteScroll {
	...
	public class InfiniteScroll : MonoBehaviour, IDropHandler {
		...
		/// </summary>
		public bool ShouldCenterContentIfItFits = false;
		...
		/// <summary>
		/// ugly hack: somehow the gameobjects are persisting on the scene/editor, even after exiting play mode
		/// </summary>
		public void ForceDestroyChildren()
		{
			Debug.Log("ForceDestroyChildren "+ _content.childCount);
			for (int i = _content.childCount - 1; i > 0; i--) //keep the first item, which is the prefab
			{
		...
		void UpdateHorizontal () {
			...
			float currentContentWidth = _content.rect.width;
			if (ShouldCenterContentIfItFits && currentContentWidth < Screen.width)
			{
				//Debug.Log("shouldCenterContentIfItFits? YES!!! screen width:" + Screen.width + "  --> content width:" + currentContentWidth);
				_scroll.enabled = false;
				//_content.localPosition = new Vector3( (Screen.width - currentContentWidth) * .5f, _content.localPosition.y, _content.localPosition.z); //this centers but adds no spacing between items
				_content.sizeDelta = new Vector2(Screen.width, _content.rect.height);
                int numItems = _scroll.content.transform.childCount;
				float itemWidth = _scroll.content.GetChild(0).GetComponent<RectTransform>().rect.width;
				float newPositionSpacing = ((Screen.width - currentContentWidth) / numItems); //- itemWidth;
				float newPosition = (itemWidth + newPositionSpacing) * .5f;

				for (int i = 0; i < numItems; i++)
				{
					Transform item = _scroll.content.GetChild(i);
					item.localPosition = new Vector3(newPosition, item.localPosition.y, item.localPosition.z);
					newPosition += itemWidth + newPositionSpacing;
				}
			}
			else
			{
			...
			if (_positions.Count != 0 && _widths.Count != 0)
				itemPosition = Mathf.Abs (_positions[_previousPosition]) + _widths[_previousPosition];
		...
		void OnScrollChange (Vector2 vector) {
			if (ShouldCenterContentIfItFits)
				return;
		...
		public void InitData (int count, RectTransform referenceRectTransform) {
			if (Type == 0) {
				InitVertical (count);
			} else {
				InitHorizontal (count, referenceRectTransform);
			}
		}
		...
		/// </summary>
		/// <param name="count">Item count</param>
		void InitHorizontal (int count, RectTransform referenceRectTransform) {

			//compare referenceRectTransform and _content
			//Debug.Log("referenceRectTransform - y:"+referenceRectTransform.rect.y);
			//Debug.Log("_content: " + _content);
			//_content = referenceRectTransform;

			float width = CalcSizesPositions (count);
			CreateViews (referenceRectTransform);
			_previousPosition = 0;
			_count = count;
			_content.sizeDelta = new Vector2 (width, _content.sizeDelta.y);
			Vector2 pos = _content.anchoredPosition;
			//Debug.Log("_content.anchoredPosition - y:" + pos.y);
			Vector2 size = Vector2.zero;
			pos.x = 0f;
			//_content.anchoredPosition = pos;
			int x = LeftPadding;
			...
			for (int i = 0; i < _views.Length; i++) {
				...
				//pos = referenceRectTransform.anchoredPosition; //_rects[i].anchoredPosition;
				//pos.x = _positions[i];
				//pos.y = 0f;
				//Debug.Log("_rects[i].anchoredPosition - y:" + pos.y);
				//_rects[i].anchoredPosition = pos;
				if (_rects != null)
				{
					_rects[i].SetLeft(x);
					_rects[i].SetRight(-x);
					_rects[i].SetTop(0);
					_rects[i].SetWidth(_widths[i]);
					//Debug.Log("InitHorizontal - Width " + i + ": " + _widths[i]);
				}
                //else Debug.Log("Prevented Exception - InitHorizontal - Width " + i);
		...
		float CalcSizesPositionsHorizontal (int count) {
			//Debug.Log("CalcSizesPositionsHorizontal - count: "+count);
			if (_widths == null)
				Init();
			_widths.Clear ();
			_positions.Clear ();
		...
		public void RecycleAll () {
			...
			Debug.Log("Recycling.. "+ _views.Length);
		...
		void CreateViews (RectTransform referenceRectTransform) {
			if (Type == 0) {
				CreateViewsVertical ();
			} else {
				CreateViewsHorizontal (referenceRectTransform);
			}
		}
		...
		void CreateViewsHorizontal () {
			...
			width = _widths.Count != 0 ? width / _widths.Count : 0;
			int fillCount = width!=0 ? Mathf.RoundToInt (_container.width / width) + 4 : 0;
			_views = new GameObject[fillCount];
			for (int i = 0; i < fillCount; i++) {
				clone = (GameObject) Instantiate (Prefab, Vector3.zero, Quaternion.identity);
				clone.transform.SetParent (_content);
		...
		void CreateViewsHorizontal(RectTransform referenceRectTransform)
		{
			if (_views != null)
			{
				return;
			...
			for (int i = 0; i < fillCount; i++)
			{
				/*GameObject templateItem4 = Instantiate(templateItem);
				templateItem4.transform.parent = templateItem.transform.parent;
				templateItem4.transform.GetComponent<RectTransform>().anchoredPosition = templateItem.transform.GetComponent<RectTransform>().anchoredPosition;
				templateItem4.transform.GetComponent<RectTransform>().SetLeft(600);
				templateItem4.transform.GetComponent<RectTransform>().SetRight(-600);
				templateItem4.transform.GetComponent<RectTransform>().SetTop(0);*/

				//clone = (GameObject)Instantiate(Prefab, Vector3.zero, Quaternion.identity);
				//clone.transform.SetParent(_content);
				clone = (GameObject)Instantiate(Prefab);
				clone.transform.SetParent(_content, false);
				clone.transform.GetComponent<RectTransform>().anchoredPosition = new Vector2(referenceRectTransform.anchoredPosition.x, referenceRectTransform.anchoredPosition.y); ;
				clone.transform.GetComponent<RectTransform>().SetLeft(0);
				clone.transform.GetComponent<RectTransform>().SetRight(0);
				clone.transform.GetComponent<RectTransform>().SetTop(0);
				clone.transform.GetComponent<RectTransform>().SetWidth(width/fillCount);

				//Debug.Log("InitHorizontal - Width " + i + ": " + width/fillCount);
				//clone.transform.localScale = Vector3.one;
				//clone.transform.localPosition = Vector3.zero;
				//rect = clone.GetComponent<RectTransform>();
				//rect.pivot = new Vector2(0f, 0.5f);
